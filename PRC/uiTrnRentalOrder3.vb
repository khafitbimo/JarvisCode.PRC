...
#Region " Overrides "
    ...
    Public Overrides Function btnNew_Click() As Boolean
        ...
        If Me.uiTrnRentalOrder3_GetOrderSource() = True Then
            Me.FormNewHidup()
            Me.LOCKING_FORM("NEW")
        Else
...
#Region " Layout & Init UI "
    ...
    '            Me.btnApp.Text = "Approve"
    '        Case 1
    '            Me.app = Approves.UnApproved
    '            Me.btnApp.Text = "Unapprove"
    '        Case 2
    '            Me.app = Approves.Approved
    '            Me.btnApp.Text = "Approve"
    '    End Select
    ...
    Private Function BindingStart() As Boolean
        ...
        Me.dtp_Order_setdate.DataBindings.Add(New Binding("Text", Me.tbl_TrnOrder_Temp, "order_setdate", True, DataSourceUpdateMode.OnPropertyChanged))
...
#Region " Dialoged Control "
    ...
    '======== ADD PTS 20150916 =============
    Public Shared Function SelectTopFrom(dt As DataTable, rowCount As Integer) As DataTable
                    ...
                    With dlgRequest
                        ...
                        If .DialogResult = DialogResult.OK Then
                            ...
                            With .dgvRequestList
                                ...
                                '=========== modified PTS 20150916 untuk handle tanggal kebalik=================================
                                Dim tbl_referenceselected As DataTable = New DataTable
                                tbl_referenceselected.Clear()

                                DataFill(tbl_referenceselected, "pr_TrnRequest_Select2", " request_id = '" & .Item("request_id", reqIndex).Value & "'")
                                tbl_referenceselected = SelectTopFrom(tbl_referenceselected, 1)

                                Dim preparedt As Date = tbl_referenceselected.Rows(0).Item("request_preparedt")
                                Dim useddt As Date = tbl_referenceselected.Rows(0).Item("request_useddt")
                                Dim useddt2 As Date = tbl_referenceselected.Rows(0).Item("request_useddt2")

                                Me.dtp_Order_setdate.Value = preparedt
                                Me.mtb_Order_settime.Text = Format(preparedt, "HH:mm")

                                Me.dtp_Order_utilizeddatestart.Value = useddt
                                Me.mtb_Order_utilizedtimestart.Text = Format(useddt, "HH:mm")

                                Me.dtp_Order_utilizeddateend.Value = useddt2
                                Me.mtb_Order_utilizedtimeend.Text = Format(useddt2, "HH:mm")
                                '==============================================================================================
                                ...
                                For i = 0 To Me.DgvTrnRequestdetil.Rows.Count - 1
                                    ...
                                    Else
                                        ...
                                        If newitem Then
                                            ...
                                            Me.uiTrnRentalOrder3_SetItemDetil(k, i, Me.dtp_Order_setdate.Value, Me.dtp_Order_utilizeddateend.Value)
        ...
        If tbl_TrnOrder_Temp_Changes IsNot Nothing _
            ...
            Try
                ...
                If tbl_TrnOrder_Temp_Changes IsNot Nothing Then
                    success = Me.uiTrnRentalOrder3_SaveMaster(order_id, tbl_TrnOrder_Temp_Changes, MasterDataState, dbConn1, dbTrans)
                ...
                '        End If
                '    Next
                '    success = Me.uiTrnRentalOrder3_CancelRequestDetil(request_id, requestdetil_line, tbl_TrnRequestdetil_Changes, MasterDataState, dbConn1, dbTrans)
                ...
                If tbl_TrnOrderdetiluse_Changes IsNot Nothing Then
                    For i = 0 To Me.tbl_TrnOrderdetiluse.Rows.Count - 1
                        ...
                        End If
                    Next
                    success = Me.uiTrnRentalOrder3_SaveUse(order_id, tbl_TrnOrderdetiluse_Changes, MasterDataState, dbConn1, dbTrans)
                ...
                If tbl_TrnOrderdetil_Changes IsNot Nothing Then
                    For i = 0 To Me.tbl_TrnOrderdetil.Rows.Count - 1
                        ...
                        End If
                    Next
                    success = Me.uiTrnRentalOrder3_SaveDetil(order_id, tbl_TrnOrderdetil_Changes, MasterDataState, dbConn1, dbTrans)
                ...
                If tbl_TrnOrderdetileps_Changes IsNot Nothing Then
                    For i = 0 To Me.tbl_TrnOrderdetileps.Rows.Count - 1
                        ...
                        End If
                    Next
                    success = Me.uiTrnRentalOrder3_SaveDetilEps(order_id, tbl_TrnOrderdetileps_Changes, MasterDataState, dbConn1, dbTrans)
                ...
                If tbl_TrnOrderpaymreq_Changes IsNot Nothing Then
                    success = Me.uiTrnRentalOrder3_SaveDetilPaymReq(order_id, tbl_TrnOrderpaymreq_Changes, MasterDataState, dbConn1, dbTrans)
...
save_confirmation:
                ...
                If SHOW_SAVE_CONFIRMATION Then
                    Dim dbConn2 As New OleDb.OleDbConnection(Me.DSN)
                    ...
                    Try
                        ...
                        Me.uiTrnRentalOrder3_OpenRowMaster(channel_id, order_id, dbConn2)
                        Me.uiTrnRentalOrder3_OpenRowDetil(channel_id, order_id, dbConn2)
                        Me.uiTrnRentalOrder3_OpenRowDetiluse(channel_id, order_id, dbConn2)
                        Me.uiTrnRentalOrder3_OpenRowPaymReq(channel_id, order_id, dbConn2)
                        Me.uiTrnRentalOrder3_OpenRowRequestdetil(channel_id, order_id, dbConn2)
                        Me.uiTrnRentalOrder3_DetilCalculate(Me.DgvTrnOrderdetil, Me.TotalDetil, Me.TotalDetilPPH, Me.TotalDetilPPN, Me.TotalDetilDisc)
                        Me.uiTrnRentalOrder3_TotalCalculate()
        ...
        Me.Cursor = Cursors.Arrow
    End Function
    Private Function uiTrnRentalOrder3_SaveMaster(ByRef order_id As Object, ByVal objTbl As DataTable, ByVal MasterDataState As System.Data.DataRowState, _
        ...
        Return True
    End Function
    Private Function uiTrnRentalOrder3_SaveDetil(ByRef order_id As Object, ByVal objTbl As DataTable, ByVal MasterDataState As System.Data.DataRowState, _
                                                    ByRef dbconn As OleDb.OleDbConnection, ByRef dbTrans As OleDb.OleDbTransaction) As Boolean
        'Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        Dim dbCmdInsert As OleDb.OleDbCommand
        Dim dbCmdUpdate As OleDb.OleDbCommand
    ...
    Private Function uiTrnRentalOrder3_CancelRequestDetil(ByRef request_id As Object, ByRef requestdetil_line As Object, ByVal objTbl As DataTable, ByVal MasterDataState As System.Data.DataRowState, _
                                                    ByRef dbconn As OleDb.OleDbConnection, ByRef dbTrans As OleDb.OleDbTransaction) As Boolean
        'Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        Dim dbCmdUpdate As OleDb.OleDbCommand
        Dim dbDA As OleDb.OleDbDataAdapter
        ...
        Return True
    End Function
    Private Function uiTrnRentalOrder3_SaveUse(ByRef order_id As Object, ByVal objTbl As DataTable, ByVal MasterDataState As System.Data.DataRowState, _
                                                    ByRef dbconn As OleDb.OleDbConnection, ByRef dbTrans As OleDb.OleDbTransaction) As Boolean
        'Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        Dim dbCmdInsert As OleDb.OleDbCommand
        Dim dbCmdUpdate As OleDb.OleDbCommand
        ...
        dbCmdInsert = New OleDb.OleDbCommand("pr_TrnOrderdetiluse_Insert", dbconn, dbTrans)
        dbCmdInsert.CommandType = CommandType.StoredProcedure
        ...
        dbCmdUpdate = New OleDb.OleDbCommand("pr_TrnOrderdetiluse_Update", dbconn, dbTrans)
        dbCmdUpdate.CommandType = CommandType.StoredProcedure
        ...
        dbCmdDelete = New OleDb.OleDbCommand("pr_TrnOrderdetiluse_Delete", dbconn, dbTrans)
        ...
        Return True
    End Function
    Private Function uiTrnRentalOrder3_SaveDetilPaymReq(ByRef order_id As Object, ByVal objTbl As DataTable, ByVal MasterDataState As System.Data.DataRowState, _
                                                    ByRef dbconn As OleDb.OleDbConnection, ByRef dbTrans As OleDb.OleDbTransaction) As Boolean
        'Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        Dim dbCmdInsert As OleDb.OleDbCommand
        Dim dbCmdUpdate As OleDb.OleDbCommand
    ...
    Private Function uiTrnRentalOrder3_SaveDetilEps(ByRef order_id As Object, ByVal objTbl As DataTable, ByVal MasterDataState As System.Data.DataRowState, _
                                                    ByRef dbconn As OleDb.OleDbConnection, ByRef dbTrans As OleDb.OleDbTransaction) As Boolean
        'Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        Dim dbCmdInsert As OleDb.OleDbCommand
        Dim dbCmdUpdate As OleDb.OleDbCommand
    ...
    Private Function uiTrnRentalOrder3_PrintPreview() As Boolean
        ...
        Dim frmPrint As dlgViewRptRO_3 = New dlgViewRptRO_3(Me.DSN, True)
    ...
    Private Function uiTrnRentalOrder3_Print() As Boolean
        ...
        Using dlg As New dlgViewRptRO_3(Me.DSN, False)
            dlg.SetIDCriteria(criteria, criteria_history, criteria_TOP)
            dlg.ShowDialog(Me)
        End Using
    ...
    Private Function uiTrnRentalOrder3_OpenRow_id(ByVal order_id As String, ByVal channel_id As String) As Boolean
        Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        Dim row As DataGridViewRow
        Dim i As Integer
    ...
    Private Function uiTrnRentalOrder3_FormError() As Boolean
        ...
        Try
            ...
            Dim dr() As DataRow = Me.tbl_MstPeriode.Select(String.Format("periode_id='{0}'", CType(Me.cbo_Periode_id.SelectedValue, String)))
            ...
            If Me.obj_Order_id.Text <> "" Or Me.obj_Order_id.Text <> String.Empty Then
                If Me.uiTrnRequestOrder3_CekOrderTarikRV(Me.obj_Order_id.Text) = True Then
            ...
            '========= TOP LOcking ================= 
            If Me.chk_Order_canceled.Checked = False Then
    ...
    End Function
    '=================== ADD PTS 20150205 =========================
    Private Function uiTrnRequestOrder3_CekOrderTarikRV(ByVal order_id) As Boolean
        Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        Dim dbCmd As OleDb.OleDbCommand
        Dim dbDA As OleDb.OleDbDataAdapter
        ...
        dbCmd.Parameters.Add("@channel_id", Data.OleDb.OleDbType.VarChar)
        dbCmd.Parameters("@order_id").Value = String.Format("order_id='{0}'", order_id)
        dbCmd.Parameters("@channel_id").Value = Me._CHANNEL
        dbCmd.CommandType = CommandType.StoredProcedure
        dbDA = New OleDb.OleDbDataAdapter(dbCmd)
        tbl_cekRV.Clear()
...
#End Region
    ...
    'Private Sub uiTrnRentalOrder3_FormAfterSave(ByRef id As Object, result As uiBase.FormSaveResult) Handles Me.FormAfterSave
    '    If result = FormSaveResult.SaveSuccess Then
    '        Dim canceled As Boolean = Me.tbl_TrnOrder_Temp.Rows(0)
    '    End If
    'End Sub
...
#Region "Approved"
    ...
    'Private app As Approves = Approves.UnApproved
    ...
    'Private Sub Approve_Order(ByVal sender As Object)
    ...
    '                            End If
    '                        End If
    '                    ElseIf app = Approves.UnApproved Then
    ...
    '                    Me.uiTrnRentalOrder3_Retrieve()
    '                    Me.uiTrnRentalOrder3_OpenRow_id(Me.obj_Order_id.Text, Me.cboSearchChannel.SelectedValue)
    '                Else
    '                    Me.uiTrnRentalOrder3_Retrieve()
    '                End If
...
#End Region
    ...
    Private Sub obj_Order_canceled_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk_Order_canceled.CheckedChanged
        ...
        If Me.chk_Order_canceled.Checked = True Then
            ...
            For i = 0 To Me.tbl_TrnRequestdetil.Rows.Count - 1
                Me.tbl_TrnRequestdetil.Rows(i).Item("requestdetil_ordered") = 0
            Next
        ...
        Else
            ...
            'Next
            For i = 0 To Me.tbl_TrnRequestdetil.Rows.Count - 1
                Me.tbl_TrnRequestdetil.Rows(i).Item("requestdetil_ordered") = 1
            Next
    ...
    Private Sub DgvTrnOrder_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DgvTrnOrder.SelectionChanged
        ...
        For i = 0 To Me.DgvTrnOrder.SelectedRows.Count - 1
            ...
            If Me.DgvTrnOrder.Rows.Count > 0 Then
                ...
                'Me.Approve(approve)
                '======= ADD PTS 20150813 =============CEK APA UDAH ADA BPJ ATAU ADVANCE======================
                Dim tbl_jurnal As DataTable = New DataTable
                Dim error_messagebpj As Decimal = 0
                Dim error_messageadvance As Decimal = 0
                ...
                If order_id <> String.Empty Then
                    tbl_jurnal.Clear()
                    Me.DataFill(tbl_jurnal, "to_TrnOrderdetil_SelectJurnal", String.Format("order_id = '{0}'", order_id))

                    tbl_orderAdvance.Clear()
                    Me.DataFill(tbl_orderAdvance, "pr_TrnOrderdetilAdvanceForOrder_Select", String.Format("order_id = '{0}' AND isadvance <> 0 and advance_canceled =0", order_id, 0))
...
#Region "Approval_Order"
    ...
    Private Function Order_Approval(ByVal approval_status As String, ByVal order_id As String) As Boolean
        ...
        Me.uiTrnRentalOrder3_OpenRow_id(order_id, Me._CHANNEL)
...
#Region "Cancel_Order"
    ...
    Private Function Order_Cancel(ByVal order_id As String) As Boolean
        ...
        Finally
            ...
            dbConn.Close()
        End Try
        Me.uiTrnRentalOrder3_OpenRow_id(Me.obj_Order_id.Text, Me._CHANNEL)
    End Function
