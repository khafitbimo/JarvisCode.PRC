...
#Region " UI and Layout "
    ...
    Private Function FormatDgvTrnRequest(ByRef objDgv As System.Windows.Forms.DataGridView) As Boolean
        ...
        Dim cExpired As System.Windows.Forms.DataGridViewTextBoxColumn = New System.Windows.Forms.DataGridViewTextBoxColumn
        ...
        cExpired.Name = "expired"
        cExpired.HeaderText = "Expired"
        cExpired.DataPropertyName = "expired"
        cExpired.Width = 90
        cExpired.Visible = False
        cExpired.ReadOnly = False
...
#End Region
    ...
    Private Sub dlgGetRequest_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.cboexpired.SelectedIndex = 0
        Me.btnLoadData.PerformClick()
    End Sub
    ...
    Private Sub btnOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOK.Click
        If ProgType = "NP" Then
            ...
            If CheckBedaCurrency() = False Then
                MsgBox("Currency can't be different.", MsgBoxStyle.Exclamation)
                Exit Sub
            End If
        ...
        If ProgType = "PG" Then
            ' Check Expired Request Jika 1 (sudah expired), 0 (belum expired)
            '===============================================================================
            Dim expired As String = Me.dgvRequestList.Item("expired", Me.dgvRequestList.CurrentRow.Index).Value
            If expired = "1" Then
                MsgBox("Request is Expired", MsgBoxStyle.Exclamation)
                Exit Sub
            End If
            '===============================================================================
        End If
    ...
    '========= ADD PTS 20171103 =========
    Private Function CheckBedaCurrency() As Boolean
        Dim rows() As DataRow = Me.tbl_RequestDetilSelect.Select("requestdetil_ordered = 1")
        Dim currency_id As String
        Dim currency_idtemp As String

        For i As Integer = 0 To rows.Length - 1
            currency_id = rows(i).Item("currency_id")

            For j As Integer = 0 To rows.Length - 1
                currency_idtemp = rows(j).Item("currency_id")

                If currency_id <> currency_idtemp Then
                    Return False
                End If
            Next
        Next
    ...
    Private Sub BtnCheckAll_Click(sender As Object, e As EventArgs) Handles BtnCheckAll.Click
        If Me.cboexpired.SelectedItem.ToString = "Expired" Then
            Exit Sub
        End If

        For Each viewRow As DataGridViewRow In Me.dgvRequestList.Rows
            viewRow.Cells("requestdetil_ordered").Value = True
        Next
    End Sub

    Private Sub cboexpired_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboexpired.SelectedIndexChanged
        'Dim criteria As String = ""

        'If Me.ProgType = "PG" Then
        '    If Me.chk_status.Checked Then
        '        If cboexpired.SelectedItem.ToString = "Not Expired" Then
        '            criteria = String.Format("expired = {0}", 0)
        '            Me.tbl_RequestSelect.DefaultView.RowFilter = criteria
        '        Else
        '            criteria = String.Format("expired = {0}", 1)
        '            Me.tbl_RequestSelect.DefaultView.RowFilter = criteria
        '        End If
        '    End If
        'Else
        ...
        '            criteria = String.Format("expired = {0}", 0)
        '            Me.tbl_RequestDetilSelect.DefaultView.RowFilter = criteria
        '            Me.dgvRequestList.ReadOnly = False
        '        Else
        '            criteria = String.Format("expired = {0}", 1)
        '            Me.tbl_RequestDetilSelect.DefaultView.RowFilter = criteria
        '            Me.dgvRequestList.ReadOnly = True
        '        End If
        '    End If
        'End If'''
    End Sub
    ...
    Private Sub btnLoadData_Click(sender As Object, e As EventArgs) Handles btnLoadData.Click
        'If Me.tbl_RequestSelect.Rows.Count <> 0 Then
        Dim criteria As String = ""

        If Me.chk_request_id.Checked Then
            If criteria <> "" Then
                criteria = criteria + " AND " + String.Format("request_id LIKE '%{0}%'", obj_request_id.Text)
            Else
                criteria = String.Format("request_id LIKE '%{0}%'", obj_request_id.Text)
            End If
        End If
        ...
        If Me.chk_rekanan.Checked Then
            If criteria <> "" Then
                criteria = criteria + " AND " + String.Format("rekanan_name LIKE '%{0}%'", obj_rekanan.Text)
            Else
                criteria = String.Format("rekanan_name LIKE '%{0}%'", obj_rekanan.Text)
            End If
        End If
        ...
        If Me.chk_status.Checked Then
            If cboexpired.SelectedItem.ToString = "Not Expired" Then
                If criteria <> "" Then
                    criteria = criteria + " AND " + String.Format("expired = {0}", 0)
                Else
                    criteria = String.Format("expired = {0}", 0)
                End If
                Me.dgvRequestList.ReadOnly = False
            Else
                If criteria <> "" Then
                    criteria = criteria + " AND " + String.Format("expired = {0}", 1)
                Else
                    criteria = String.Format("expired = {0}", 1)
                End If
                Me.dgvRequestList.ReadOnly = True
